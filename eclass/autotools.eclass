...
if [[ -n ${WANT_AUTOMAKE} ]] ; then
	case ${WANT_AUTOMAKE} in
		...
		none) ;;
		latest) _automake_atom="|| ( `printf '>=app-build/automake-%s:%s ' ${_LATEST_AUTOMAKE[@]/:/ }` )" ;;
...
if [[ -n ${WANT_AUTOCONF} ]] ; then
	case ${WANT_AUTOCONF} in
		...
		2.1)        _autoconf_atom=">=app-build/autoconf-2.13-r7:2.1" ;;
		# if you change the "latest" version here, change also autotools_env_setup
		latest|2.5) _autoconf_atom=">=app-build/autoconf-2.71-r5" ;;
		*)          die "Invalid WANT_AUTOCONF value '${WANT_AUTOCONF}'" ;;
	esac
	export WANT_AUTOCONF
fi

_libtool_atom=">=app-build/libtool-2.4.7"
