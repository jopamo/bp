...
RDEPEND="
	>=dev-pypi/certifi-2017.4.17[${PYTHON_USEDEP}]
	<dev-pypi/charset-normalizer-4[${PYTHON_USEDEP}]
	<dev-pypi/idna-4[${PYTHON_USEDEP}]
	<dev-pypi/urllib3-3[${PYTHON_USEDEP}]
	socks5? ( >=dev-pypi/pysocks-1.5.6[${PYTHON_USEDEP}] )
"

BDEPEND="
	test? (
		>=dev-pypi/pysocks-1.5.6[${PYTHON_USEDEP}]
		test-rust? (
...
python_test() {
	...
	if ! has_version "dev-pypi/trustme[${PYTHON_USEDEP}]"; then
		EPYTEST_DESELECT+=(
