...
db_ver_to_slot() {
	...
	# older lib-sys/db might have been using this but it's no longer
...
db_includedir() {
	...
	if [ $# -eq 0 ]; then
		...
		if [ -d "${EPREFIX}/usr/include/db${VER}" ]; then
			...
			eerror "lib-sys/db package requested, but headers not found" >&2
			return 1
		fi
	else
		#arguments given
		for x in $@
		do
			if VER=$(db_findver "=lib-sys/db-${x}*") &&
...
db_libname() {
	...
	if [ $# -eq 0 ]; then
		...
		if [ -e "${EPREFIX}/usr/$(get_libdir)/libdb-${VER}$(get_libname)" ]; then
			...
			eerror "lib-sys/db package requested, but library not found" >&2
			return 1
		fi
	else
		#arguments given
