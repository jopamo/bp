...
RDEPEND="
	>=dev-pypi/asgiref-3.4.0[${PYTHON_USEDEP}]
	>=dev-pypi/click-7.0[${PYTHON_USEDEP}]
	>=dev-pypi/h11-0.8[${PYTHON_USEDEP}]
"
BDEPEND="
	test? (
		dev-py/a2wsgi[${PYTHON_USEDEP}]
		>=dev-py/httptools-0.6.3[${PYTHON_USEDEP}]
		>=dev-pypi/httpx-0.28[${PYTHON_USEDEP}]
		dev-pypi/python-dotenv[${PYTHON_USEDEP}]
		dev-pypi/pyyaml[${PYTHON_USEDEP}]
		dev-pypi/typing-extensions[${PYTHON_USEDEP}]
		>=dev-pypi/websockets-10.4[${PYTHON_USEDEP}]
		dev-pypi/wsproto[${PYTHON_USEDEP}]
		test-rust? (
			app-crypto/cryptography[${PYTHON_USEDEP}]
			dev-pypi/trustme[${PYTHON_USEDEP}]
			dev-py/watchfiles[${PYTHON_USEDEP}]
		)
	)
"
...
pkg_postinst() {
	optfeature "auto reload on file changes" dev-py/watchfiles
}
