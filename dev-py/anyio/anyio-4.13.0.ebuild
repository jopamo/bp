...
RDEPEND="
	>=dev-py/idna-2.8[${PYTHON_USEDEP}]
	>=dev-py/truststore-0.9.1[${PYTHON_USEDEP}]
	$(python_gen_cond_dep '
		>=dev-py/typing-extensions-4.5[${PYTHON_USEDEP}]
	' 3.{11..12})
"
...
BDEPEND="
	...
	test? (
		...
		>=dev-py/exceptiongroup-1.2.0[${PYTHON_USEDEP}]
		>=dev-py/psutil-5.9[${PYTHON_USEDEP}]
		>=dev-py/trustme-1.0.0[${PYTHON_USEDEP}]
		$(python_gen_cond_dep '
			>=dev-py/trio-0.32.0[${PYTHON_USEDEP}]
		' 3.{11..14})
		amd64? (
			$(python_gen_cond_dep '
...
python_test() {
	...
	local filter=()
	if ! has_version ">=dev-py/trio-0.26.1[${PYTHON_USEDEP}]"; then
		filter+=( -k "not trio" )
		EPYTEST_DESELECT+=(
