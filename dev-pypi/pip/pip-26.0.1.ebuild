...
# please bump dev-pypi/ensurepip-pip along with this package!
...
RDEPEND="
	>=dev-pypi/cachecontrol-0.14.3[${PYTHON_USEDEP}]
	...
	>=dev-pypi/requests-2.32.4[${PYTHON_USEDEP}]
...
BDEPEND="
	...
	test? (
		$(python_gen_cond_dep '
			...
			dev-pypi/scripttest[${PYTHON_USEDEP}]
			>=dev-pypi/setuptools-70.1.0[${PYTHON_USEDEP}]
			dev-pypi/virtualenv[${PYTHON_USEDEP}]
			dev-pypi/werkzeug[${PYTHON_USEDEP}]
			test-rust? (
...
python_configure() {
	if use test && has_version "dev-pypi/pip[${PYTHON_USEDEP}]"; then
		"${EPYTHON}" -m pip check ||
