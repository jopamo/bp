...
RDEPEND="
	dev-py/anyio[${PYTHON_USEDEP}]
	dev-py/certifi[${PYTHON_USEDEP}]
	=dev-py/httpcore-1*[${PYTHON_USEDEP}]
	dev-py/idna[${PYTHON_USEDEP}]
	cli? (
		=dev-py/click-8*[${PYTHON_USEDEP}]
		=dev-py/pygments-2*[${PYTHON_USEDEP}]
...
python_test() {
	...
	if ! has_version "dev-py/trio[${PYTHON_USEDEP}]"; then
...
pkg_postinst() {
	optfeature "HTTP/2 support" dev-py/h2
	optfeature "SOCKS proxy support" dev-py/socksio
	optfeature "Decoding for brotli compressed responses" dev-py/brotlicffi
}
