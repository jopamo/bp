...
enewuser() {
	...
	local create_home=1 force_uid root=${ROOT:-}
	while [[ ${1-} == -* ]]; do
		case ${1-} in
			-F) force_uid=1 ;;
			-M) create_home= ;;
	...
	local user=${1-} uid=${2-} shell=${3-} home=${4-} groups=${5-}
	...
	# ---- groups -------------------------------------------------------------
	local defgroup= exgroups= g
	local -a groups_arr=()
	if [[ -n ${groups} ]]; then
		IFS=',' read -r -a groups_arr <<< "${groups}"
	...
	if [[ -n ${create_home} && ${home} != /dev/null && ! -e ${root%/}${home} ]]; then
		elog " - creating ${home}"
		mkdir -p "${root%/}${home}"
...
esethome() {
	...
	if [[ ${home} != /dev/null && ! -e ${root%/}${home} ]]; then
		mkdir -p "${root%/}${home}"
...
esetshell() {
	...
	local user=$1 shell=$2 root=${ROOT:-}
	[[ -n $(egetent passwd "${user}") ]] || { ewarn "user missing"; return 1; }
...
esetcomment() {
	...
	local user=$1 comment=$2 root=${ROOT:-}
	[[ -n $(egetent passwd "${user}") ]] || { ewarn "user missing"; return 1; }
...
esetgroups() {
	...
	local user=$1 groups_raw=$2 root=${ROOT:-}
	[[ -n $(egetent passwd "${user}") ]] || { ewarn "user missing"; return 1; }
