...
dotnet_multilib_comply() {
	...
	if [[ -d "${ED}/usr/lib" && "lib" != "lib" ]]
	then
		if ! [[ -d "${ED}"/usr/"lib" ]]
		then
			mkdir "${ED}"/usr/"lib" || die "Couldn't mkdir ${ED}/usr/lib"
		fi
		${mv_command} "${ED}"/usr/lib/* "${ED}"/usr/"lib"/ || die "Moving files into correct libdir failed"
		rm -rf "${ED}"/usr/lib
		for dir in "${ED}"/usr/"lib"/pkgconfig "${ED}"/usr/share/pkgconfig
		do

			if [[ -d "${dir}" && "$(find "${dir}" -name '*.pc')" != "" ]]
			then
				pushd "${dir}" &> /dev/null
				sed  -i -r -e 's:/(lib)([^a-zA-Z0-9]|$):/'"lib"'\2:g' \
					*.pc \
