# Distributed under the terms of the GNU General Public License v2

SNAPSHOT=5d3c4817f1619213951dbf15031bad04acb88392
SHORT=${SNAPSHOT:0:9}

inherit flag-o-matic go-module

DESCRIPTION="GitHub CLI"
HOMEPAGE="https://github.com/cli/cli"
SRC_URI="https://github.com/cli/cli/archive/${SNAPSHOT}.tar.gz -> ${PN}-${SNAPSHOT}.tar.gz"

# lockstep-go-managed: true
# lockstep-go-deps: begin
EGO_SUM=(
	"charm.land/bubbles/v2 v2.1.1"
	"charm.land/bubbles/v2 v2.1.1/go.mod"
	"charm.land/bubbletea/v2 v2.0.8"
	"charm.land/bubbletea/v2 v2.0.8/go.mod"
	"charm.land/huh/v2 v2.0.3"
	"charm.land/huh/v2 v2.0.3/go.mod"
	"charm.land/lipgloss/v2 v2.0.6"
	"charm.land/lipgloss/v2 v2.0.6/go.mod"
	"cloud.google.com/go v0.123.0"
	"cloud.google.com/go v0.123.0/go.mod"
	"cloud.google.com/go/auth v0.20.0"
	"cloud.google.com/go/auth v0.20.0/go.mod"
	"cloud.google.com/go/auth/oauth2adapt v0.2.8"
	"cloud.google.com/go/auth/oauth2adapt v0.2.8/go.mod"
	"cloud.google.com/go/compute/metadata v0.9.0"
	"cloud.google.com/go/compute/metadata v0.9.0/go.mod"
	"cloud.google.com/go/iam v1.11.0"
	"cloud.google.com/go/iam v1.11.0/go.mod"
	"cloud.google.com/go/kms v1.31.0"
	"cloud.google.com/go/kms v1.31.0/go.mod"
	"cloud.google.com/go/longrunning v1.0.0"
	"cloud.google.com/go/longrunning v1.0.0/go.mod"
	"dario.cat/mergo v1.0.2"
	"dario.cat/mergo v1.0.2/go.mod"
	"filippo.io/edwards25519 v1.2.0"
	"filippo.io/edwards25519 v1.2.0/go.mod"
	"filippo.io/mldsa v0.0.0-20260215214346-43d0283efc3e"
	"filippo.io/mldsa v0.0.0-20260215214346-43d0283efc3e/go.mod"
	"github.com/AdamKorcz/go-fuzz-headers-1 v0.0.0-20230919221257-8b5d3ce2d11d"
	"github.com/AdamKorcz/go-fuzz-headers-1 v0.0.0-20230919221257-8b5d3ce2d11d/go.mod"
	"github.com/AlecAivazis/survey/v2 v2.3.7"
	"github.com/AlecAivazis/survey/v2 v2.3.7/go.mod"
	"github.com/Azure/azure-sdk-for-go/sdk/azcore v1.21.1"
	"github.com/Azure/azure-sdk-for-go/sdk/azcore v1.21.1/go.mod"
	"github.com/Azure/azure-sdk-for-go/sdk/azidentity v1.13.1"
	"github.com/Azure/azure-sdk-for-go/sdk/azidentity v1.13.1/go.mod"
	"github.com/Azure/azure-sdk-for-go/sdk/internal v1.12.0"
	"github.com/Azure/azure-sdk-for-go/sdk/internal v1.12.0/go.mod"
	"github.com/Azure/azure-sdk-for-go/sdk/security/keyvault/azkeys v1.5.0"
	"github.com/Azure/azure-sdk-for-go/sdk/security/keyvault/azkeys v1.5.0/go.mod"
	"github.com/Azure/azure-sdk-for-go/sdk/security/keyvault/internal v1.2.0"
	"github.com/Azure/azure-sdk-for-go/sdk/security/keyvault/internal v1.2.0/go.mod"
	"github.com/AzureAD/microsoft-authentication-library-for-go v1.7.0"
	"github.com/AzureAD/microsoft-authentication-library-for-go v1.7.0/go.mod"
	"github.com/MakeNowJust/heredoc v1.0.0"
	"github.com/MakeNowJust/heredoc v1.0.0/go.mod"
	"github.com/Masterminds/goutils v1.1.1"
	"github.com/Masterminds/goutils v1.1.1/go.mod"
	"github.com/Masterminds/semver/v3 v3.4.0"
	"github.com/Masterminds/semver/v3 v3.4.0/go.mod"
	"github.com/Masterminds/sprig/v3 v3.3.0"
	"github.com/Masterminds/sprig/v3 v3.3.0/go.mod"
	"github.com/Netflix/go-expect v0.0.0-20220104043353-73e0943537d2"
	"github.com/Netflix/go-expect v0.0.0-20220104043353-73e0943537d2/go.mod"
	"github.com/alecthomas/assert/v2 v2.11.0"
	"github.com/alecthomas/assert/v2 v2.11.0/go.mod"
	"github.com/alecthomas/chroma/v2 v2.19.0"
	"github.com/alecthomas/chroma/v2 v2.19.0/go.mod"
	"github.com/alecthomas/repr v0.4.0"
	"github.com/alecthomas/repr v0.4.0/go.mod"
	"github.com/asaskevich/govalidator v0.0.0-20230301143203-a9d515a09cc2"
	"github.com/asaskevich/govalidator v0.0.0-20230301143203-a9d515a09cc2/go.mod"
	"github.com/atotto/clipboard v0.1.4"
	"github.com/atotto/clipboard v0.1.4/go.mod"
	"github.com/aws/aws-sdk-go-v2 v1.41.9"
	"github.com/aws/aws-sdk-go-v2 v1.41.9/go.mod"
	"github.com/aws/aws-sdk-go-v2/config v1.32.20"
	"github.com/aws/aws-sdk-go-v2/config v1.32.20/go.mod"
	"github.com/aws/aws-sdk-go-v2/credentials v1.19.19"
	"github.com/aws/aws-sdk-go-v2/credentials v1.19.19/go.mod"
	"github.com/aws/aws-sdk-go-v2/feature/ec2/imds v1.18.25"
	"github.com/aws/aws-sdk-go-v2/feature/ec2/imds v1.18.25/go.mod"
	"github.com/aws/aws-sdk-go-v2/internal/configsources v1.4.25"
	"github.com/aws/aws-sdk-go-v2/internal/configsources v1.4.25/go.mod"
	"github.com/aws/aws-sdk-go-v2/internal/endpoints/v2 v2.7.25"
	"github.com/aws/aws-sdk-go-v2/internal/endpoints/v2 v2.7.25/go.mod"
	"github.com/aws/aws-sdk-go-v2/internal/v4a v1.4.26"
	"github.com/aws/aws-sdk-go-v2/internal/v4a v1.4.26/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/internal/accept-encoding v1.13.10"
	"github.com/aws/aws-sdk-go-v2/service/internal/accept-encoding v1.13.10/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/internal/presigned-url v1.13.25"
	"github.com/aws/aws-sdk-go-v2/service/internal/presigned-url v1.13.25/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/kms v1.52.0"
	"github.com/aws/aws-sdk-go-v2/service/kms v1.52.0/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/signin v1.1.1"
	"github.com/aws/aws-sdk-go-v2/service/signin v1.1.1/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/sso v1.30.19"
	"github.com/aws/aws-sdk-go-v2/service/sso v1.30.19/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/ssooidc v1.36.2"
	"github.com/aws/aws-sdk-go-v2/service/ssooidc v1.36.2/go.mod"
	"github.com/aws/aws-sdk-go-v2/service/sts v1.42.3"
	"github.com/aws/aws-sdk-go-v2/service/sts v1.42.3/go.mod"
	"github.com/aws/smithy-go v1.26.0"
	"github.com/aws/smithy-go v1.26.0/go.mod"
	"github.com/aymanbagabas/go-osc52/v2 v2.0.1"
	"github.com/aymanbagabas/go-osc52/v2 v2.0.1/go.mod"
	"github.com/aymanbagabas/go-udiff v0.4.1"
	"github.com/aymanbagabas/go-udiff v0.4.1/go.mod"
	"github.com/aymerick/douceur v0.2.0"
	"github.com/aymerick/douceur v0.2.0/go.mod"
	"github.com/blang/semver v3.5.1+incompatible"
	"github.com/blang/semver v3.5.1+incompatible/go.mod"
	"github.com/briandowns/spinner v1.23.2"
	"github.com/briandowns/spinner v1.23.2/go.mod"
	"github.com/catppuccin/go v0.3.0"
	"github.com/catppuccin/go v0.3.0/go.mod"
	"github.com/cenkalti/backoff/v4 v4.3.0"
	"github.com/cenkalti/backoff/v4 v4.3.0/go.mod"
	"github.com/cenkalti/backoff/v5 v5.0.3"
	"github.com/cenkalti/backoff/v5 v5.0.3/go.mod"
	"github.com/cespare/xxhash/v2 v2.3.0"
	"github.com/cespare/xxhash/v2 v2.3.0/go.mod"
	"github.com/charmbracelet/colorprofile v0.4.3"
	"github.com/charmbracelet/colorprofile v0.4.3/go.mod"
	"github.com/charmbracelet/glamour v0.10.0"
	"github.com/charmbracelet/glamour v0.10.0/go.mod"
	"github.com/charmbracelet/lipgloss v1.1.1-0.20250404203927-76690c660834"
	"github.com/charmbracelet/lipgloss v1.1.1-0.20250404203927-76690c660834/go.mod"
	"github.com/charmbracelet/ultraviolet v0.0.0-20260811164956-006e29f97886"
	"github.com/charmbracelet/ultraviolet v0.0.0-20260811164956-006e29f97886/go.mod"
	"github.com/charmbracelet/x/ansi v0.11.8"
	"github.com/charmbracelet/x/ansi v0.11.8/go.mod"
	"github.com/charmbracelet/x/cellbuf v0.0.15"
	"github.com/charmbracelet/x/cellbuf v0.0.15/go.mod"
	"github.com/charmbracelet/x/conpty v0.1.1"
	"github.com/charmbracelet/x/conpty v0.1.1/go.mod"
	"github.com/charmbracelet/x/errors v0.0.0-20240508181413-e8d8b6e2de86"
	"github.com/charmbracelet/x/errors v0.0.0-20240508181413-e8d8b6e2de86/go.mod"
	"github.com/charmbracelet/x/exp/golden v0.0.0-20250806222409-83e3a29d542f"
	"github.com/charmbracelet/x/exp/golden v0.0.0-20250806222409-83e3a29d542f/go.mod"
	"github.com/charmbracelet/x/exp/ordered v0.1.0"
	"github.com/charmbracelet/x/exp/ordered v0.1.0/go.mod"
	"github.com/charmbracelet/x/exp/slice v0.0.0-20250630141444-821143405392"
	"github.com/charmbracelet/x/exp/slice v0.0.0-20250630141444-821143405392/go.mod"
	"github.com/charmbracelet/x/exp/strings v0.0.0-20250630141444-821143405392"
	"github.com/charmbracelet/x/exp/strings v0.0.0-20250630141444-821143405392/go.mod"
	"github.com/charmbracelet/x/term v0.2.2"
	"github.com/charmbracelet/x/term v0.2.2/go.mod"
	"github.com/charmbracelet/x/termios v0.1.1"
	"github.com/charmbracelet/x/termios v0.1.1/go.mod"
	"github.com/charmbracelet/x/windows v0.2.2"
	"github.com/charmbracelet/x/windows v0.2.2/go.mod"
	"github.com/charmbracelet/x/xpty v0.1.3"
	"github.com/charmbracelet/x/xpty v0.1.3/go.mod"
	"github.com/cli/browser v1.0.0/go.mod"
	"github.com/cli/browser v1.3.0"
	"github.com/cli/browser v1.3.0/go.mod"
	"github.com/cli/go-gh/v2 v2.13.0"
	"github.com/cli/go-gh/v2 v2.13.0/go.mod"
	"github.com/cli/go-internal v0.0.0-20241025142207-6c48bcd5ce24"
	"github.com/cli/go-internal v0.0.0-20241025142207-6c48bcd5ce24/go.mod"
	"github.com/cli/oauth v1.2.2"
	"github.com/cli/oauth v1.2.2/go.mod"
	"github.com/cli/safeexec v1.0.0/go.mod"
	"github.com/cli/safeexec v1.0.1"
	"github.com/cli/safeexec v1.0.1/go.mod"
	"github.com/cli/shurcooL-graphql v0.0.4"
	"github.com/cli/shurcooL-graphql v0.0.4/go.mod"
	"github.com/clipperhouse/displaywidth v0.11.0"
	"github.com/clipperhouse/displaywidth v0.11.0/go.mod"
	"github.com/clipperhouse/uax29/v2 v2.7.0"
	"github.com/clipperhouse/uax29/v2 v2.7.0/go.mod"
	"github.com/codahale/rfc6979 v0.0.0-20141003034818-6a90f24967eb"
	"github.com/codahale/rfc6979 v0.0.0-20141003034818-6a90f24967eb/go.mod"
	"github.com/coreos/go-oidc/v3 v3.17.0"
	"github.com/coreos/go-oidc/v3 v3.17.0/go.mod"
	"github.com/cpuguy83/go-md2man/v2 v2.0.6/go.mod"
	"github.com/cpuguy83/go-md2man/v2 v2.0.7"
	"github.com/cpuguy83/go-md2man/v2 v2.0.7/go.mod"
	"github.com/creack/pty v1.1.17/go.mod"
	"github.com/creack/pty v1.1.24"
	"github.com/creack/pty v1.1.24/go.mod"
	"github.com/cyberphone/json-canonicalization v0.0.0-20241213102144-19d51d7fe467"
	"github.com/cyberphone/json-canonicalization v0.0.0-20241213102144-19d51d7fe467/go.mod"
	"github.com/danieljoos/wincred v1.2.3"
	"github.com/danieljoos/wincred v1.2.3/go.mod"
	"github.com/davecgh/go-spew v1.1.0/go.mod"
	"github.com/davecgh/go-spew v1.1.1/go.mod"
	"github.com/digitorus/pkcs7 v0.0.0-20230713084857-e76b763bdc49/go.mod"
	"github.com/digitorus/pkcs7 v0.0.0-20230818184609-3a137a874352"
	"github.com/digitorus/pkcs7 v0.0.0-20230818184609-3a137a874352/go.mod"
	"github.com/digitorus/timestamp v0.0.0-20250524132541-c45532741eea"
	"github.com/digitorus/timestamp v0.0.0-20250524132541-c45532741eea/go.mod"
	"github.com/distribution/reference v0.6.0"
	"github.com/distribution/reference v0.6.0/go.mod"
	"github.com/dlclark/regexp2 v1.11.5"
	"github.com/dlclark/regexp2 v1.11.5/go.mod"
	"github.com/docker/cli v29.6.2+incompatible"
	"github.com/docker/cli v29.6.2+incompatible/go.mod"
	"github.com/docker/docker-credential-helpers v0.9.3"
	"github.com/docker/docker-credential-helpers v0.9.3/go.mod"
	"github.com/dustin/go-humanize v1.0.1"
	"github.com/dustin/go-humanize v1.0.1/go.mod"
	"github.com/fatih/color v1.18.0"
	"github.com/fatih/color v1.18.0/go.mod"
	"github.com/felixge/httpsnoop v1.0.4"
	"github.com/felixge/httpsnoop v1.0.4/go.mod"
	"github.com/frankban/quicktest v1.14.6"
	"github.com/frankban/quicktest v1.14.6/go.mod"
	"github.com/gabriel-vasile/mimetype v1.4.15"
	"github.com/gabriel-vasile/mimetype v1.4.15/go.mod"
	"github.com/gdamore/encoding v1.0.1"
	"github.com/gdamore/encoding v1.0.1/go.mod"
	"github.com/gdamore/tcell/v2 v2.13.10"
	"github.com/gdamore/tcell/v2 v2.13.10/go.mod"
	"github.com/go-chi/chi/v5 v5.3.0"
	"github.com/go-chi/chi/v5 v5.3.0/go.mod"
	"github.com/go-jose/go-jose/v4 v4.1.4"
	"github.com/go-jose/go-jose/v4 v4.1.4/go.mod"
	"github.com/go-logr/logr v1.2.2/go.mod"
	"github.com/go-logr/logr v1.4.4"
	"github.com/go-logr/logr v1.4.4/go.mod"
	"github.com/go-logr/stdr v1.2.2"
	"github.com/go-logr/stdr v1.2.2/go.mod"
	"github.com/go-openapi/analysis v0.25.5"
	"github.com/go-openapi/analysis v0.25.5/go.mod"
	"github.com/go-openapi/errors v0.22.8"
	"github.com/go-openapi/errors v0.22.8/go.mod"
	"github.com/go-openapi/jsonpointer v1.0.0"
	"github.com/go-openapi/jsonpointer v1.0.0/go.mod"
	"github.com/go-openapi/jsonreference v1.0.0"
	"github.com/go-openapi/jsonreference v1.0.0/go.mod"
	"github.com/go-openapi/loads v0.25.0"
	"github.com/go-openapi/loads v0.25.0/go.mod"
	"github.com/go-openapi/runtime v0.33.0"
	"github.com/go-openapi/runtime v0.33.0/go.mod"
	"github.com/go-openapi/runtime/server-middleware v0.30.0"
	"github.com/go-openapi/runtime/server-middleware v0.30.0/go.mod"
	"github.com/go-openapi/spec v0.22.9"
	"github.com/go-openapi/spec v0.22.9/go.mod"
	"github.com/go-openapi/strfmt v0.27.0"
	"github.com/go-openapi/strfmt v0.27.0/go.mod"
	"github.com/go-openapi/swag v0.26.1"
	"github.com/go-openapi/swag v0.26.1/go.mod"
	"github.com/go-openapi/swag/cmdutils v0.27.0"
	"github.com/go-openapi/swag/cmdutils v0.27.0/go.mod"
	"github.com/go-openapi/swag/conv v0.27.3"
	"github.com/go-openapi/swag/conv v0.27.3/go.mod"
	"github.com/go-openapi/swag/fileutils v0.27.3"
	"github.com/go-openapi/swag/fileutils v0.27.3/go.mod"
	"github.com/go-openapi/swag/jsonname v0.26.1"
	"github.com/go-openapi/swag/jsonname v0.26.1/go.mod"
	"github.com/go-openapi/swag/jsonutils v0.27.3"
	"github.com/go-openapi/swag/jsonutils v0.27.3/go.mod"
	"github.com/go-openapi/swag/jsonutils/fixtures_test v0.27.3"
	"github.com/go-openapi/swag/jsonutils/fixtures_test v0.27.3/go.mod"
	"github.com/go-openapi/swag/loading v0.27.3"
	"github.com/go-openapi/swag/loading v0.27.3/go.mod"
	"github.com/go-openapi/swag/mangling v0.27.3"
	"github.com/go-openapi/swag/mangling v0.27.3/go.mod"
	"github.com/go-openapi/swag/netutils v0.27.0"
	"github.com/go-openapi/swag/netutils v0.27.0/go.mod"
	"github.com/go-openapi/swag/pools v0.27.3"
	"github.com/go-openapi/swag/pools v0.27.3/go.mod"
	"github.com/go-openapi/swag/stringutils v0.27.3"
	"github.com/go-openapi/swag/stringutils v0.27.3/go.mod"
	"github.com/go-openapi/swag/typeutils v0.27.3"
	"github.com/go-openapi/swag/typeutils v0.27.3/go.mod"
	"github.com/go-openapi/swag/yamlutils v0.27.3"
	"github.com/go-openapi/swag/yamlutils v0.27.3/go.mod"
	"github.com/go-openapi/testify/enable/yaml/v2 v2.6.0"
	"github.com/go-openapi/testify/enable/yaml/v2 v2.6.0/go.mod"
	"github.com/go-openapi/testify/v2 v2.6.0"
	"github.com/go-openapi/testify/v2 v2.6.0/go.mod"
	"github.com/go-openapi/validate v0.26.1"
	"github.com/go-openapi/validate v0.26.1/go.mod"
	"github.com/go-test/deep v1.1.1"
	"github.com/go-test/deep v1.1.1/go.mod"
	"github.com/go-viper/mapstructure/v2 v2.5.0"
	"github.com/go-viper/mapstructure/v2 v2.5.0/go.mod"
	"github.com/godbus/dbus/v5 v5.2.2"
	"github.com/godbus/dbus/v5 v5.2.2/go.mod"
	"github.com/golang-jwt/jwt/v5 v5.3.1"
	"github.com/golang-jwt/jwt/v5 v5.3.1/go.mod"
	"github.com/golang/protobuf v1.5.4"
	"github.com/golang/protobuf v1.5.4/go.mod"
	"github.com/google/certificate-transparency-go v1.3.3"
	"github.com/google/certificate-transparency-go v1.3.3/go.mod"
	"github.com/google/go-cmp v0.6.0/go.mod"
	"github.com/google/go-cmp v0.7.0"
	"github.com/google/go-cmp v0.7.0/go.mod"
	"github.com/google/go-containerregistry v0.21.9"
	"github.com/google/go-containerregistry v0.21.9/go.mod"
	"github.com/google/s2a-go v0.1.9"
	"github.com/google/s2a-go v0.1.9/go.mod"
	"github.com/google/shlex v0.0.0-20191202100458-e7afc7fbc510"
	"github.com/google/shlex v0.0.0-20191202100458-e7afc7fbc510/go.mod"
	"github.com/google/trillian v1.7.3"
	"github.com/google/trillian v1.7.3/go.mod"
	"github.com/google/uuid v1.6.0"
	"github.com/google/uuid v1.6.0/go.mod"
	"github.com/googleapis/enterprise-certificate-proxy v0.3.16"
	"github.com/googleapis/enterprise-certificate-proxy v0.3.16/go.mod"
	"github.com/googleapis/gax-go/v2 v2.22.0"
	"github.com/googleapis/gax-go/v2 v2.22.0/go.mod"
	"github.com/gorilla/css v1.0.1"
	"github.com/gorilla/css v1.0.1/go.mod"
	"github.com/gorilla/websocket v1.5.3"
	"github.com/gorilla/websocket v1.5.3/go.mod"
	"github.com/grpc-ecosystem/go-grpc-middleware v1.4.0"
	"github.com/grpc-ecosystem/go-grpc-middleware v1.4.0/go.mod"
	"github.com/grpc-ecosystem/grpc-gateway/v2 v2.29.0"
	"github.com/grpc-ecosystem/grpc-gateway/v2 v2.29.0/go.mod"
	"github.com/h2non/parth v0.0.0-20190131123155-b4df798d6542"
	"github.com/h2non/parth v0.0.0-20190131123155-b4df798d6542/go.mod"
	"github.com/hashicorp/errwrap v1.1.0"
	"github.com/hashicorp/errwrap v1.1.0/go.mod"
	"github.com/hashicorp/go-cleanhttp v0.5.2"
	"github.com/hashicorp/go-cleanhttp v0.5.2/go.mod"
	"github.com/hashicorp/go-multierror v1.1.1"
	"github.com/hashicorp/go-multierror v1.1.1/go.mod"
	"github.com/hashicorp/go-retryablehttp v0.7.8"
	"github.com/hashicorp/go-retryablehttp v0.7.8/go.mod"
	"github.com/hashicorp/go-rootcerts v1.0.2"
	"github.com/hashicorp/go-rootcerts v1.0.2/go.mod"
	"github.com/hashicorp/go-secure-stdlib/parseutil v0.2.0"
	"github.com/hashicorp/go-secure-stdlib/parseutil v0.2.0/go.mod"
	"github.com/hashicorp/go-secure-stdlib/strutil v0.1.2"
	"github.com/hashicorp/go-secure-stdlib/strutil v0.1.2/go.mod"
	"github.com/hashicorp/go-sockaddr v1.0.7"
	"github.com/hashicorp/go-sockaddr v1.0.7/go.mod"
	"github.com/hashicorp/go-version v1.9.0"
	"github.com/hashicorp/go-version v1.9.0/go.mod"
	"github.com/hashicorp/hcl v1.0.1-vault-7"
	"github.com/hashicorp/hcl v1.0.1-vault-7/go.mod"
	"github.com/hashicorp/vault/api v1.22.0"
	"github.com/hashicorp/vault/api v1.22.0/go.mod"
	"github.com/henvic/httpretty v0.1.4"
	"github.com/henvic/httpretty v0.1.4/go.mod"
	"github.com/hexops/gotextdiff v1.0.3"
	"github.com/hexops/gotextdiff v1.0.3/go.mod"
	"github.com/hinshun/vt10x v0.0.0-20220119200601-820417d04eec"
	"github.com/hinshun/vt10x v0.0.0-20220119200601-820417d04eec/go.mod"
	"github.com/howeyc/gopass v0.0.0-20210920133722-c8aef6fb66ef"
	"github.com/howeyc/gopass v0.0.0-20210920133722-c8aef6fb66ef/go.mod"
	"github.com/huandu/xstrings v1.5.0"
	"github.com/huandu/xstrings v1.5.0/go.mod"
	"github.com/in-toto/attestation v1.2.0"
	"github.com/in-toto/attestation v1.2.0/go.mod"
	"github.com/in-toto/in-toto-golang v0.11.0"
	"github.com/in-toto/in-toto-golang v0.11.0/go.mod"
	"github.com/inconshreveable/mousetrap v1.1.0"
	"github.com/inconshreveable/mousetrap v1.1.0/go.mod"
	"github.com/itchyny/gojq v0.12.17"
	"github.com/itchyny/gojq v0.12.17/go.mod"
	"github.com/itchyny/timefmt-go v0.1.6"
	"github.com/itchyny/timefmt-go v0.1.6/go.mod"
	"github.com/jedisct1/go-minisign v0.0.0-20241212093149-d2f9f49435c7"
	"github.com/jedisct1/go-minisign v0.0.0-20241212093149-d2f9f49435c7/go.mod"
	"github.com/jellydator/ttlcache/v3 v3.4.0"
	"github.com/jellydator/ttlcache/v3 v3.4.0/go.mod"
	"github.com/joho/godotenv v1.5.1"
	"github.com/joho/godotenv v1.5.1/go.mod"
	"github.com/kballard/go-shellquote v0.0.0-20180428030007-95032a82bc51"
	"github.com/kballard/go-shellquote v0.0.0-20180428030007-95032a82bc51/go.mod"
	"github.com/klauspost/compress v1.19.2"
	"github.com/klauspost/compress v1.19.2/go.mod"
	"github.com/kr/pretty v0.3.1"
	"github.com/kr/pretty v0.3.1/go.mod"
	"github.com/kr/text v0.2.0"
	"github.com/kr/text v0.2.0/go.mod"
	"github.com/kylelemons/godebug v1.1.0"
	"github.com/kylelemons/godebug v1.1.0/go.mod"
	"github.com/leaanthony/go-ansi-parser v1.6.1"
	"github.com/leaanthony/go-ansi-parser v1.6.1/go.mod"
	"github.com/letsencrypt/boulder v0.20260309.0"
	"github.com/letsencrypt/boulder v0.20260309.0/go.mod"
	"github.com/lucasb-eyer/go-colorful v1.4.1"
	"github.com/lucasb-eyer/go-colorful v1.4.1/go.mod"
	"github.com/mattn/go-colorable v0.1.15"
	"github.com/mattn/go-colorable v0.1.15/go.mod"
	"github.com/mattn/go-colorable v0.1.2/go.mod"
	"github.com/mattn/go-isatty v0.0.24"
	"github.com/mattn/go-isatty v0.0.24/go.mod"
	"github.com/mattn/go-isatty v0.0.8/go.mod"
	"github.com/mattn/go-runewidth v0.0.12/go.mod"
	"github.com/mattn/go-runewidth v0.0.16/go.mod"
	"github.com/mattn/go-runewidth v0.0.24"
	"github.com/mattn/go-runewidth v0.0.24/go.mod"
	"github.com/mgutz/ansi v0.0.0-20170206155736-9520e82c474b/go.mod"
	"github.com/mgutz/ansi v0.0.0-20200706080929-d51e80ef957d"
	"github.com/mgutz/ansi v0.0.0-20200706080929-d51e80ef957d/go.mod"
	"github.com/microcosm-cc/bluemonday v1.0.27"
	"github.com/microcosm-cc/bluemonday v1.0.27/go.mod"
	"github.com/microsoft/dev-tunnels v0.1.27"
	"github.com/microsoft/dev-tunnels v0.1.27/go.mod"
	"github.com/mitchellh/copystructure v1.2.0"
	"github.com/mitchellh/copystructure v1.2.0/go.mod"
	"github.com/mitchellh/go-homedir v1.1.0"
	"github.com/mitchellh/go-homedir v1.1.0/go.mod"
	"github.com/mitchellh/hashstructure/v2 v2.0.2"
	"github.com/mitchellh/hashstructure/v2 v2.0.2/go.mod"
	"github.com/mitchellh/mapstructure v1.5.0"
	"github.com/mitchellh/mapstructure v1.5.0/go.mod"
	"github.com/mitchellh/reflectwalk v1.0.2"
	"github.com/mitchellh/reflectwalk v1.0.2/go.mod"
	"github.com/muesli/cancelreader v0.2.2"
	"github.com/muesli/cancelreader v0.2.2/go.mod"
	"github.com/muesli/reflow v0.3.0"
	"github.com/muesli/reflow v0.3.0/go.mod"
	"github.com/muesli/termenv v0.16.0"
	"github.com/muesli/termenv v0.16.0/go.mod"
	"github.com/muhammadmuzzammil1998/jsonc v1.0.0"
	"github.com/muhammadmuzzammil1998/jsonc v1.0.0/go.mod"
	"github.com/natefinch/atomic v1.0.1"
	"github.com/natefinch/atomic v1.0.1/go.mod"
	"github.com/nbio/st v0.0.0-20140626010706-e9e8d9816f32"
	"github.com/nbio/st v0.0.0-20140626010706-e9e8d9816f32/go.mod"
	"github.com/oklog/ulid/v2 v2.1.1"
	"github.com/oklog/ulid/v2 v2.1.1/go.mod"
	"github.com/opencontainers/go-digest v1.0.0"
	"github.com/opencontainers/go-digest v1.0.0/go.mod"
	"github.com/opencontainers/image-spec v1.1.1"
	"github.com/opencontainers/image-spec v1.1.1/go.mod"
	"github.com/opentracing/opentracing-go v1.2.0"
	"github.com/opentracing/opentracing-go v1.2.0/go.mod"
	"github.com/pborman/getopt v0.0.0-20170112200414-7148bc3a4c30/go.mod"
	"github.com/pkg/browser v0.0.0-20240102092130-5ac0b6a4141c"
	"github.com/pkg/browser v0.0.0-20240102092130-5ac0b6a4141c/go.mod"
	"github.com/pkg/errors v0.9.1"
	"github.com/pkg/errors v0.9.1/go.mod"
	"github.com/pmezard/go-difflib v1.0.0/go.mod"
	"github.com/rivo/tview v0.42.0"
	"github.com/rivo/tview v0.42.0/go.mod"
	"github.com/rivo/uniseg v0.1.0/go.mod"
	"github.com/rivo/uniseg v0.2.0/go.mod"
	"github.com/rivo/uniseg v0.4.7"
	"github.com/rivo/uniseg v0.4.7/go.mod"
	"github.com/rodaine/table v1.3.0"
	"github.com/rodaine/table v1.3.0/go.mod"
	"github.com/rogpeppe/go-internal v1.14.1"
	"github.com/rogpeppe/go-internal v1.14.1/go.mod"
	"github.com/russross/blackfriday/v2 v2.1.0"
	"github.com/russross/blackfriday/v2 v2.1.0/go.mod"
	"github.com/ryanuber/go-glob v1.0.0"
	"github.com/ryanuber/go-glob v1.0.0/go.mod"
	"github.com/sassoftware/relic v7.2.1+incompatible"
	"github.com/sassoftware/relic v7.2.1+incompatible/go.mod"
	"github.com/sassoftware/relic/v7 v7.6.2"
	"github.com/sassoftware/relic/v7 v7.6.2/go.mod"
	"github.com/secure-systems-lab/go-securesystemslib v0.11.0"
	"github.com/secure-systems-lab/go-securesystemslib v0.11.0/go.mod"
	"github.com/sergi/go-diff v1.4.0"
	"github.com/sergi/go-diff v1.4.0/go.mod"
	"github.com/shibumi/go-pathspec v1.3.0"
	"github.com/shibumi/go-pathspec v1.3.0/go.mod"
	"github.com/shopspring/decimal v1.4.0"
	"github.com/shopspring/decimal v1.4.0/go.mod"
	"github.com/shurcooL/githubv4 v0.0.0-20240727222349-48295856cce7"
	"github.com/shurcooL/githubv4 v0.0.0-20240727222349-48295856cce7/go.mod"
	"github.com/shurcooL/graphql v0.0.0-20230722043721-ed46e5a46466"
	"github.com/shurcooL/graphql v0.0.0-20230722043721-ed46e5a46466/go.mod"
	"github.com/sigstore/protobuf-specs v0.5.1"
	"github.com/sigstore/protobuf-specs v0.5.1/go.mod"
	"github.com/sigstore/rekor v1.5.3"
	"github.com/sigstore/rekor v1.5.3/go.mod"
	"github.com/sigstore/rekor-tiles/v2 v2.3.0"
	"github.com/sigstore/rekor-tiles/v2 v2.3.0/go.mod"
	"github.com/sigstore/sigstore v1.10.8"
	"github.com/sigstore/sigstore v1.10.8/go.mod"
	"github.com/sigstore/sigstore-go v1.3.0"
	"github.com/sigstore/sigstore-go v1.3.0/go.mod"
	"github.com/sigstore/sigstore/pkg/signature/kms/aws v1.10.8"
	"github.com/sigstore/sigstore/pkg/signature/kms/aws v1.10.8/go.mod"
	"github.com/sigstore/sigstore/pkg/signature/kms/azure v1.10.8"
	"github.com/sigstore/sigstore/pkg/signature/kms/azure v1.10.8/go.mod"
	"github.com/sigstore/sigstore/pkg/signature/kms/gcp v1.10.8"
	"github.com/sigstore/sigstore/pkg/signature/kms/gcp v1.10.8/go.mod"
	"github.com/sigstore/sigstore/pkg/signature/kms/hashivault v1.10.8"
	"github.com/sigstore/sigstore/pkg/signature/kms/hashivault v1.10.8/go.mod"
	"github.com/sigstore/timestamp-authority/v2 v2.1.3"
	"github.com/sigstore/timestamp-authority/v2 v2.1.3/go.mod"
	"github.com/sirupsen/logrus v1.9.4"
	"github.com/sirupsen/logrus v1.9.4/go.mod"
	"github.com/spf13/cast v1.10.0"
	"github.com/spf13/cast v1.10.0/go.mod"
	"github.com/spf13/cobra v1.10.2"
	"github.com/spf13/cobra v1.10.2/go.mod"
	"github.com/spf13/pflag v1.0.10"
	"github.com/spf13/pflag v1.0.10/go.mod"
	"github.com/spf13/pflag v1.0.9/go.mod"
	"github.com/stretchr/objx v0.1.0/go.mod"
	"github.com/stretchr/objx v0.4.0/go.mod"
	"github.com/stretchr/objx v0.5.0/go.mod"
	"github.com/stretchr/objx v0.5.2/go.mod"
	"github.com/stretchr/objx v0.5.3"
	"github.com/stretchr/objx v0.5.3/go.mod"
	"github.com/stretchr/testify v1.12.1"
	"github.com/stretchr/testify v1.12.1/go.mod"
	"github.com/stretchr/testify v1.3.0/go.mod"
	"github.com/stretchr/testify v1.6.1/go.mod"
	"github.com/stretchr/testify v1.7.1/go.mod"
	"github.com/stretchr/testify v1.8.0/go.mod"
	"github.com/stretchr/testify v1.8.4/go.mod"
	"github.com/stretchr/testify v1.9.0/go.mod"
	"github.com/theupdateframework/go-tuf v0.7.0"
	"github.com/theupdateframework/go-tuf v0.7.0/go.mod"
	"github.com/theupdateframework/go-tuf/v2 v2.4.2"
	"github.com/theupdateframework/go-tuf/v2 v2.4.2/go.mod"
	"github.com/thlib/go-timezone-local v0.0.6"
	"github.com/thlib/go-timezone-local v0.0.6/go.mod"
	"github.com/tink-crypto/tink-go-awskms/v3 v3.0.0"
	"github.com/tink-crypto/tink-go-awskms/v3 v3.0.0/go.mod"
	"github.com/tink-crypto/tink-go-gcpkms/v2 v2.3.0"
	"github.com/tink-crypto/tink-go-gcpkms/v2 v2.3.0/go.mod"
	"github.com/tink-crypto/tink-go-hcvault/v2 v2.5.0"
	"github.com/tink-crypto/tink-go-hcvault/v2 v2.5.0/go.mod"
	"github.com/tink-crypto/tink-go/v2 v2.7.0"
	"github.com/tink-crypto/tink-go/v2 v2.7.0/go.mod"
	"github.com/titanous/rocacheck v0.0.0-20171023193734-afe73141d399"
	"github.com/titanous/rocacheck v0.0.0-20171023193734-afe73141d399/go.mod"
	"github.com/transparency-dev/formats v0.1.1"
	"github.com/transparency-dev/formats v0.1.1/go.mod"
	"github.com/transparency-dev/merkle v0.0.2"
	"github.com/transparency-dev/merkle v0.0.2/go.mod"
	"github.com/twitchtv/twirp v8.1.3+incompatible"
	"github.com/twitchtv/twirp v8.1.3+incompatible/go.mod"
	"github.com/vmihailenco/msgpack/v5 v5.4.1"
	"github.com/vmihailenco/msgpack/v5 v5.4.1/go.mod"
	"github.com/vmihailenco/tagparser/v2 v2.0.0"
	"github.com/vmihailenco/tagparser/v2 v2.0.0/go.mod"
	"github.com/xo/terminfo v0.0.0-20220910002029-abceb7e1c41e"
	"github.com/xo/terminfo v0.0.0-20220910002029-abceb7e1c41e/go.mod"
	"github.com/youmark/pkcs8 v0.0.0-20240726163527-a2c0da244d78"
	"github.com/youmark/pkcs8 v0.0.0-20240726163527-a2c0da244d78/go.mod"
	"github.com/yuin/goldmark v1.4.13/go.mod"
	"github.com/yuin/goldmark v1.8.5"
	"github.com/yuin/goldmark v1.8.5/go.mod"
	"github.com/yuin/goldmark-emoji v1.0.6"
	"github.com/yuin/goldmark-emoji v1.0.6/go.mod"
	"github.com/zalando/go-keyring v0.2.8"
	"github.com/zalando/go-keyring v0.2.8/go.mod"
	"go.opentelemetry.io/auto/sdk v1.2.1"
	"go.opentelemetry.io/auto/sdk v1.2.1/go.mod"
	"go.opentelemetry.io/contrib/instrumentation/google.golang.org/grpc/otelgrpc v0.67.0"
	"go.opentelemetry.io/contrib/instrumentation/google.golang.org/grpc/otelgrpc v0.67.0/go.mod"
	"go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp v0.67.0"
	"go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp v0.67.0/go.mod"
	"go.opentelemetry.io/otel v1.44.0"
	"go.opentelemetry.io/otel v1.44.0/go.mod"
	"go.opentelemetry.io/otel/metric v1.44.0"
	"go.opentelemetry.io/otel/metric v1.44.0/go.mod"
	"go.opentelemetry.io/otel/sdk v1.44.0"
	"go.opentelemetry.io/otel/sdk v1.44.0/go.mod"
	"go.opentelemetry.io/otel/sdk/metric v1.44.0"
	"go.opentelemetry.io/otel/sdk/metric v1.44.0/go.mod"
	"go.opentelemetry.io/otel/trace v1.44.0"
	"go.opentelemetry.io/otel/trace v1.44.0/go.mod"
	"go.step.sm/crypto v0.77.7"
	"go.step.sm/crypto v0.77.7/go.mod"
	"go.uber.org/goleak v1.3.0"
	"go.uber.org/goleak v1.3.0/go.mod"
	"go.uber.org/multierr v1.11.0"
	"go.uber.org/multierr v1.11.0/go.mod"
	"go.uber.org/zap v1.28.0"
	"go.uber.org/zap v1.28.0/go.mod"
	"go.yaml.in/yaml/v2 v2.4.4"
	"go.yaml.in/yaml/v2 v2.4.4/go.mod"
	"go.yaml.in/yaml/v3 v3.0.4/go.mod"
	"go.yaml.in/yaml/v3 v3.0.5"
	"go.yaml.in/yaml/v3 v3.0.5/go.mod"
	"golang.org/x/crypto v0.0.0-20190308221718-c2843e01d9a2/go.mod"
	"golang.org/x/crypto v0.0.0-20210921155107-089bfa567519/go.mod"
	"golang.org/x/crypto v0.55.0"
	"golang.org/x/crypto v0.55.0/go.mod"
	"golang.org/x/exp v0.0.0-20251023183803-a4bb9ffd2546"
	"golang.org/x/exp v0.0.0-20251023183803-a4bb9ffd2546/go.mod"
	"golang.org/x/mod v0.38.0"
	"golang.org/x/mod v0.38.0/go.mod"
	"golang.org/x/mod v0.6.0-dev.0.20220419223038-86c51ed26bb4/go.mod"
	"golang.org/x/mod v0.8.0/go.mod"
	"golang.org/x/net v0.0.0-20190620200207-3b0461eec859/go.mod"
	"golang.org/x/net v0.0.0-20210226172049-e18ecbb05110/go.mod"
	"golang.org/x/net v0.0.0-20220722155237-a158d28d115b/go.mod"
	"golang.org/x/net v0.57.0"
	"golang.org/x/net v0.57.0/go.mod"
	"golang.org/x/net v0.6.0/go.mod"
	"golang.org/x/oauth2 v0.36.0"
	"golang.org/x/oauth2 v0.36.0/go.mod"
	"golang.org/x/sync v0.0.0-20190423024810-112230192c58/go.mod"
	"golang.org/x/sync v0.0.0-20220722155255-886fb9371eb4/go.mod"
	"golang.org/x/sync v0.1.0/go.mod"
	"golang.org/x/sync v0.22.0"
	"golang.org/x/sync v0.22.0/go.mod"
	"golang.org/x/sys v0.0.0-20190215142949-d0b11bdaac8a/go.mod"
	"golang.org/x/sys v0.0.0-20190222072716-a9d3bda3a223/go.mod"
	"golang.org/x/sys v0.0.0-20201119102817-f84b799fce68/go.mod"
	"golang.org/x/sys v0.0.0-20210615035016-665e8c7367d1/go.mod"
	"golang.org/x/sys v0.0.0-20210831042530-f4d43177bf5e/go.mod"
	"golang.org/x/sys v0.0.0-20220520151302-bc2c85ada10a/go.mod"
	"golang.org/x/sys v0.0.0-20220722155257-8c9f86f7a55f/go.mod"
	"golang.org/x/sys v0.47.0"
	"golang.org/x/sys v0.47.0/go.mod"
	"golang.org/x/sys v0.5.0/go.mod"
	"golang.org/x/term v0.0.0-20201126162022-7de9c90e9dd1/go.mod"
	"golang.org/x/term v0.0.0-20210927222741-03fcf44c2211/go.mod"
	"golang.org/x/term v0.45.0"
	"golang.org/x/term v0.45.0/go.mod"
	"golang.org/x/term v0.5.0/go.mod"
	"golang.org/x/text v0.14.0/go.mod"
	"golang.org/x/text v0.3.0/go.mod"
	"golang.org/x/text v0.3.3/go.mod"
	"golang.org/x/text v0.3.7/go.mod"
	"golang.org/x/text v0.4.0/go.mod"
	"golang.org/x/text v0.41.0"
	"golang.org/x/text v0.41.0/go.mod"
	"golang.org/x/text v0.7.0/go.mod"
	"golang.org/x/time v0.15.0"
	"golang.org/x/time v0.15.0/go.mod"
	"golang.org/x/tools v0.0.0-20180917221912-90fa682c2a6e/go.mod"
	"golang.org/x/tools v0.0.0-20191119224855-298f0cb1881e/go.mod"
	"golang.org/x/tools v0.1.12/go.mod"
	"golang.org/x/tools v0.48.0"
	"golang.org/x/tools v0.48.0/go.mod"
	"golang.org/x/tools v0.6.0/go.mod"
	"golang.org/x/xerrors v0.0.0-20190717185122-a985d3407aa7/go.mod"
	"gonum.org/v1/gonum v0.17.0"
	"gonum.org/v1/gonum v0.17.0/go.mod"
	"google.golang.org/api v0.283.0"
	"google.golang.org/api v0.283.0/go.mod"
	"google.golang.org/genproto v0.0.0-20260319201613-d00831a3d3e7"
	"google.golang.org/genproto v0.0.0-20260319201613-d00831a3d3e7/go.mod"
	"google.golang.org/genproto/googleapis/api v0.0.0-20260526163538-3dc84a4a5aaa"
	"google.golang.org/genproto/googleapis/api v0.0.0-20260526163538-3dc84a4a5aaa/go.mod"
	"google.golang.org/genproto/googleapis/rpc v0.0.0-20260526163538-3dc84a4a5aaa"
	"google.golang.org/genproto/googleapis/rpc v0.0.0-20260526163538-3dc84a4a5aaa/go.mod"
	"google.golang.org/grpc v1.83.0"
	"google.golang.org/grpc v1.83.0/go.mod"
	"google.golang.org/protobuf v1.36.12"
	"google.golang.org/protobuf v1.36.12/go.mod"
	"gopkg.in/check.v1 v0.0.0-20161208181325-20d25e280405/go.mod"
	"gopkg.in/check.v1 v1.0.0-20201130134442-10cb98267c6c"
	"gopkg.in/check.v1 v1.0.0-20201130134442-10cb98267c6c/go.mod"
	"gopkg.in/h2non/gock.v1 v1.1.2"
	"gopkg.in/h2non/gock.v1 v1.1.2/go.mod"
	"gopkg.in/yaml.v3 v3.0.0-20200313102051-9f266ea9e77c/go.mod"
	"gopkg.in/yaml.v3 v3.0.1"
	"gopkg.in/yaml.v3 v3.0.1/go.mod"
	"gotest.tools/v3 v3.5.2"
	"gotest.tools/v3 v3.5.2/go.mod"
	"k8s.io/klog/v2 v2.140.0"
	"k8s.io/klog/v2 v2.140.0/go.mod"
	"sigs.k8s.io/yaml v1.6.0"
	"sigs.k8s.io/yaml v1.6.0/go.mod"
	"software.sslmate.com/src/go-pkcs12 v0.4.0"
	"software.sslmate.com/src/go-pkcs12 v0.4.0/go.mod"
)

_GOMODULE_GOPROXY_BASEURI="https://proxy.golang.org"
go-module_set_globals

SRC_URI+=" ${EGO_SUM_SRC_URI}"
# lockstep-go-deps: end
S="${WORKDIR}/cli-${SNAPSHOT}"

LICENSE="MIT Apache-2.0 BSD BSD-2 MPL-2.0"
SLOT="0"
KEYWORDS="amd64 arm64"
BDEPEND+=" >=app-lang/go-1.26.20260305:="

RESTRICT="test"

# Build against the packaged Go toolchain and the fetched module distfiles only.
export GOSUMDB=off
export GOTOOLCHAIN=local

src_compile() {
	export GH_VERSION="snapshot-${PV}-${SHORT}"

	filter-flags "-flto*"
	filter-flags "-ggdb3"
	unset LDFLAGS

	emake

	einfo "Building man pages"
	emake manpages
}

src_install() {
	dobin bin/gh

	doman share/man/man?/gh*.?
}
