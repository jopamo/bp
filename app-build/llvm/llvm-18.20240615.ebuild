...
src_configure() {
	...
	if use bootstrap; then
		...
		mycmakeargs+=("${common_all[@]}" "${common_late[@]}" "${stage[@]}")
	else
		mycmakeargs+=("${common_all[@]}" "${common_late[@]}" "${stage[@]}" "${final[@]}")
	fi
...
src_install() {
	...
	if ! use bootstrap; then
		rm -f "${ED}/usr/lib/libunwind.so.1" "${ED}/usr/lib/libunwind.so"
		rm -f "${ED}/usr/lib/libc++.so.1" "${ED}/usr/lib/libc++.so"
		rm -f "${ED}/usr/lib/libc++abi.so.1" "${ED}/usr/lib/libc++abi.so"

		dosym -r "/usr/lib/libunwind.so.1.0" "/usr/lib/libunwind.so.1"
		dosym -r "/usr/lib/libunwind.so.1" "/usr/lib/libunwind.so"

		dosym -r "/usr/lib/libc++.so.1.0" "/usr/lib/libc++.so.1"
		dosym -r "/usr/lib/libc++.so.1" "/usr/lib/libc++.so"

		dosym -r "/usr/lib/libc++abi.so.1.0" "/usr/lib/libc++abi.so.1"
		dosym -r "/usr/lib/libc++abi.so.1" "/usr/lib/libc++abi.so"
	fi
}
