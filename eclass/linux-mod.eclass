...
SLOT="0"
RDEPEND="${MODULES_OPTIONAL_USE}${MODULES_OPTIONAL_USE:+? (} sys-app/kmod ${MODULES_OPTIONAL_USE:+)}"
DEPEND="${RDEPEND}
	...
	do
		...
		then
			...
			eval "emake HOSTCC=\"$(tc-getBUILD_CC)\" \
						...
						${BUILD_PARAMS} \
						${BUILD_TARGETS} " \
				|| die "Unable to emake HOSTCC="$(tc-getBUILD_CC)" CROSS_COMPILE=${CHOST}- ${BUILD_FIXES} ${BUILD_PARAMS} ${BUILD_TARGETS}"
			cd "${OLDPWD}"
			touch "${srcdir}"/.built
		fi
	done
