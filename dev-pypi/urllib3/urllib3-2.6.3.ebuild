...
RDEPEND="
	>=dev-pypi/pysocks-1.5.8[${PYTHON_USEDEP}]
	<dev-pypi/pysocks-2.0[${PYTHON_USEDEP}]
	brotli? ( >=dev-pypi/brotlicffi-1.2.0.0[${PYTHON_USEDEP}] )
	http2? (
		<dev-pypi/h2-5[${PYTHON_USEDEP}]
		>=dev-pypi/h2-4[${PYTHON_USEDEP}]
	)
	zstd? (
		$(python_gen_cond_dep '
			>=dev-pypi/backports-zstd-1.0.0[${PYTHON_USEDEP}]
		' 3.{11..13})
	)
"
BDEPEND="
	...
	test? (
		$(python_gen_cond_dep "
			...
			dev-pypi/quart[\${PYTHON_USEDEP}]
			dev-py/quart-trio[\${PYTHON_USEDEP}]
			dev-pypi/trio[\${PYTHON_USEDEP}]
			>=dev-pypi/tornado-4.2.1[\${PYTHON_USEDEP}]
			>=dev-pypi/trustme-0.5.3[\${PYTHON_USEDEP}]
		" "${PYTHON_TESTED[@]}")
		$(python_gen_cond_dep '
			>=dev-pypi/backports-zstd-1.0.0[${PYTHON_USEDEP}]
		' 3.{11..13})
