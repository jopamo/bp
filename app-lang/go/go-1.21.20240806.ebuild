...
src_prepare() {
	default
	# app-build/gcc[go-bootstrap] installs a private GOROOT-like bootstrap tree
	# here so upstream make.bash can use gccgo without needing /usr/bin/go.
}

src_compile() {
	export GOROOT_BOOTSTRAP="${BROOT%/}/usr/lib/gccgo"
