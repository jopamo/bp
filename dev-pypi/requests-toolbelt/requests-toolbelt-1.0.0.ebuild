...
RDEPEND="
	<dev-pypi/requests-3.0.0[${PYTHON_USEDEP}]
"
BDEPEND="
	test? (
		dev-pypi/betamax[${PYTHON_USEDEP}]
		test-rust? (
...
python_test() {
	...
	if ! has_version "app-crypto/cryptography[${PYTHON_USEDEP}]" ||
		! has_version "dev-pypi/pyopenssl[${PYTHON_USEDEP}]" ||
		! has_version "dev-pypi/trustme[${PYTHON_USEDEP}]"
	then
		EPYTEST_IGNORE+=(
