...
# @FUNCTION: doicon_all
# @USAGE: doicon_all <icon>
# @DESCRIPTION:
#  Takes a single large icon file <icon> and automatically generates the
...
doicon_all() {
	...
	for s in "${SIZES[@]}"; do
		...
		magick "${src}" -resize "${s}x${s}" "${out}" \
			|| die "Resizing ${src} to ${s}x${s} failed"

		# Now install that scaled PNG using doicon, specifying the size
		doicon -s "${s}" "${out}"
	done
}
