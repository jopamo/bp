...
RDEPEND="
	>=dev-pypi/iniconfig-1.0.1[${PYTHON_USEDEP}]
	>=dev-pypi/packaging-22[${PYTHON_USEDEP}]
	<dev-pypi/pluggy-2[${PYTHON_USEDEP}]
	>=dev-pypi/pluggy-1.5.0[${PYTHON_USEDEP}]
	>=dev-pypi/pygments-2.7.2[${PYTHON_USEDEP}]
"
BDEPEND="
	...
	test? (
		${RDEPEND}
		$(python_gen_cond_dep '
			dev-pypi/argcomplete[${PYTHON_USEDEP}]
			>=dev-pypi/attrs-19.2[${PYTHON_USEDEP}]
			>=dev-pypi/hypothesis-3.56[${PYTHON_USEDEP}]
			dev-pypi/mock[${PYTHON_USEDEP}]
			dev-pypi/pytest-xdist[${PYTHON_USEDEP}]
			dev-pypi/requests[${PYTHON_USEDEP}]
			dev-pypi/xmlschema[${PYTHON_USEDEP}]
		' "${PYTHON_TESTED[@]}")
	)
"
