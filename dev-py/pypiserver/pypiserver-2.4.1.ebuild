...
RDEPEND="
	dev-py/bottle[${PYTHON_USEDEP}]
	>=dev-py/packaging-23.2[${PYTHON_USEDEP}]
	>=dev-py/pip-7[${PYTHON_USEDEP}]
	$(python_gen_cond_dep '
		dev-py/importlib-resources[${PYTHON_USEDEP}]
	' 3.11)
"
...
BDEPEND="
	...
	test? (
		>=dev-py/build-1.2.0[${PYTHON_USEDEP}]
		>=dev-py/passlib-1.6[${PYTHON_USEDEP}]
		dev-py/setuptools[${PYTHON_USEDEP}]
		dev-py/webtest[${PYTHON_USEDEP}]
	)
"
...
python_test() {
	...
	if ! has_version "dev-py/twine[${PYTHON_USEDEP}]"; then
		EPYTEST_DESELECT+=(
